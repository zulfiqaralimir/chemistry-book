...
<figure className="my-12 rounded-lg border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-900">
  <img src="/topics/atomic-structure/aufbau-principle.png" alt="Aufbau Principle: order of filling of s, p, d, and f orbitals by increasing energy, following the n+l rule, shown as diagonal fill lines from 1s through 7p" className="mx-auto max-w-full" />
</figure>
...
**The Maximum Capacity Formula: $2n^2$**

Each shell's maximum electron capacity follows a simple formula:

$$
\text{maximum electrons in a shell} = 2n^2, \quad \text{where } n \text{ is the shell number}
$$

- Shell 1 ($n=1$): $2 \times 1^2 = 2$
- Shell 2 ($n=2$): $2 \times 2^2 = 8$
- Shell 3 ($n=3$): $2 \times 3^2 = 18$
- Shell 4 ($n=4$): $2 \times 4^2 = 32$
...
This is where most students get stuck, and it's worth resolving precisely. The $2n^2$ capacity ($18$ for shell 3) and the octet target ($8$) are answering two different questions:

- $2n^2 = 18$ → the *total physical capacity* of shell 3, relevant once it's no longer the outermost shell.
- Octet rule ($8$) → a *stability target*, applying only to whichever shell is currently **outermost** — regardless of that shell's absolute maximum capacity.
...
The octet rule only ever applies to whichever shell is currently outermost — never to a shell's full $2n^2$ capacity.

**Why Does Shell 3 "Stop" at 8? Sub-Levels and Energy Ordering**

The real mechanism: each shell isn't one single unit — it's divided into **sub-levels**, named $s$, $p$, $d$, $f$, each with its own fixed capacity:
...
Shell number $n$ contains $n$ types of sub-levels, starting from $s$:

- Shell 1: $s$ only → $2$ (matches $2n^2=2$)
- Shell 2: $s+p$ → $2+6=8$ (matches $2n^2=8$)
- Shell 3: $s+p+d$ → $2+6+10=18$ (matches $2n^2=18$)
- Shell 4: $s+p+d+f$ → $2+6+10+14=32$ (matches $2n^2=32$)

Electrons fill the lowest-energy sub-level available first — and energy doesn't rise in tidy shell order. Specifically, **shell 3's $d$ sub-level sits at a higher energy than shell 4's $s$ sub-level.** So once shell 3's $s$ and $p$ sub-levels are full ($2+6=8$ electrons), the next electron skips ahead into shell 4's $s$ sub-level rather than shell 3's $d$ sub-level — which is exactly why shell 3 "stops at 8" while acting as an outer shell. Shell 3's $d$ sub-level only fills in later, once shell 4 has already started, in heavier elements.
...
Filling in true energy order: $1s^2\,2s^2\,2p^6\,3s^2\,3p^5$ ($2+2+6+2+5=17$). Grouped by shell: Shell 1 = 2, Shell 2 = 8, Shell 3 = $2+5=7$ → arrangement **2, 8, 7**. Chlorine's outer shell (shell 3) has 7 electrons — 1 short of the octet's 8 — making it highly reactive, eager to gain 1 electron.

**Worked Example — Iron (26 electrons), Past Atomic Number 20**

Filling in true energy order: $1s^2\,2s^2\,2p^6\,3s^2\,3p^6\,4s^2\,3d^6$ ($2+2+6+2+6+2+6=26$). Grouped by shell: Shell 1 = 2, Shell 2 = 8, Shell 3 = $2+6+6=14$, Shell 4 = 2 → arrangement **2, 8, 14, 2**.
...
| Atomic # | Element | Electron Configuration | Shells (filled electrons) | Last Sub-level Filled | Next Sub-level Expected |
| :-: | :-- | :-- | :-- | :-- | :-- |
| 21 | Scandium (Sc) | $1s^2$ $2s^2$ $2p^6$ $3s^2$ $3p^6$ $4s^2$ **$3d^1$** | Shell 1: 2, Shell 2: 8, Shell 3: 9, Shell 4: 2 | $3d$ — 1 of 10 electrons | $3d$ continues filling (capacity 10); after that, $4p$ (capacity 6) |
| 31 | Gallium (Ga) | $1s^2$ $2s^2$ $2p^6$ $3s^2$ $3p^6$ $4s^2$ $3d^{10}$ **$4p^1$** | Shell 1: 2, Shell 2: 8, Shell 3: 18, Shell 4: 3 | $4p$ — 1 of 6 electrons | $4p$ continues filling (capacity 6); after that, $5s$ (capacity 2) |
| 41 | Niobium (Nb) | $1s^2$ $2s^2$ $2p^6$ $3s^2$ $3p^6$ $4s^2$ $3d^{10}$ $4p^6$ $5s^2$ **$4d^3$** | Shell 1: 2, Shell 2: 8, Shell 3: 18, Shell 4: 11, Shell 5: 2 | $4d$ — 3 of 10 electrons | $4d$ continues filling (capacity 10); after that, $5p$ (capacity 6) |
| 51 | Antimony (Sb) | $1s^2$ $2s^2$ $2p^6$ $3s^2$ $3p^6$ $4s^2$ $3d^{10}$ $4p^6$ $5s^2$ $4d^{10}$ **$5p^3$** | Shell 1: 2, Shell 2: 8, Shell 3: 18, Shell 4: 18, Shell 5: 5 | $5p$ — 3 of 6 electrons | $5p$ continues filling (capacity 6); after that, $6s$ (capacity 2) |
| 61 | Promethium (Pm) | $1s^2$ $2s^2$ $2p^6$ $3s^2$ $3p^6$ $4s^2$ $3d^{10}$ $4p^6$ $5s^2$ $4d^{10}$ $5p^6$ $6s^2$ **$4f^5$** | Shell 1: 2, Shell 2: 8, Shell 3: 18, Shell 4: 23, Shell 5: 8, Shell 6: 2 | $4f$ — 5 of 14 electrons | $4f$ continues filling (capacity 14); after that, $5d$ (capacity 10) |

**How the shell totals are built:** each shell's number is the sum of its own sub-levels. For example, niobium's shell 4 = $4s^2 + 4p^6 + 4d^3 = 2+6+3 = 11$ (note: the "4" in "$4d$" refers to shell 4, not shell 5 — a detail worth double-checking carefully, since it's an easy place to slip up). Shell 5 for niobium is just $5s^2 = 2$.

**Worth flagging honestly:** real-world niobium is actually a known exception to the predicted pattern above — its true measured configuration is $4d^4\,5s^1$ instead of $4d^3\,5s^2$, due to a subtle extra-stability effect when a $d$ sub-level gets close to half-full. Several such exceptions exist among the transition metals; the Aufbau order predicts the correct configuration for the large majority of elements, with a handful of memorized special cases like this one.

**Key Takeaway**

Shells and orbits are the same idea — energy layers around the nucleus, each with a theoretical maximum capacity given by $2n^2$. Orbitals are the more precise, modern sub-regions within a shell. The octet rule doesn't care about a shell's full $2n^2$ capacity — it only asks whether the current outermost shell has reached a stable 8 (or 2, for shell 1). Shells are actually built from sub-levels ($s$, $p$, $d$, $f$), each with its own capacity, and electrons fill these in strict energy order (the Aufbau principle) — which is why shell 3 appears to "stop" at 8 while it's the outer shell (its $d$ sub-level is higher energy than shell 4's $s$ sub-level), only filling the rest of its capacity later, once it becomes an inner shell in heavier elements like iron, scandium, and beyond.

## Chapter 9 — How We Discovered Sub-Shell Energy Levels

*The story behind $s$, $p$, $d$, $f$.*

The letters $s$, $p$, $d$, $f$ get handed to chemistry students as if they were always just labels to memorize. But they weren't invented from theory first — they came from a real experimental puzzle, decades before anyone could mathematically explain why they existed. This chapter traces that story, from the original clue to the final confirmation.

**The Clue That Started It: Light From Heated Atoms Isn't Smooth**

It begins with a simple experiment: heat an element — say, sodium or hydrogen gas — until it glows, then pass that glow through a prism. If you expected a full, smooth rainbow of every color blending into the next, you'd be wrong. Instead, you get only a handful of **sharp, distinct lines** of color at very specific positions, with darkness everywhere in between. This pattern is called an **emission spectrum** — "spectrum" meaning the spread-out pattern of light by color, "emission" meaning light being given off by the glowing atoms.

This was a genuine puzzle for physicists in the late 1800s and early 1900s. If electrons could hold *any* amount of energy, heating an atom should release energy smoothly across every possible amount, producing a continuous rainbow. It didn't. Energy only ever came out in specific, fixed jumps — producing only those sharp lines and nothing in between.

**The First Big Conclusion: Energy Comes in Fixed Packets**

This single observation forced a major rethink. It told scientists that electrons can't hold just any energy — they're restricted to specific, fixed energy levels. Light is only released when an electron drops from one fixed level down to another, and the color of light released corresponds exactly to the energy difference between those two levels.

This is the origin of the word **"quantum"** — meaning a fixed, discrete amount. Energy isn't smooth and continuous around an atom; it comes in fixed packets, or "quanta." This single realization is what first pushed scientists toward the idea of shells (fixed energy levels) at all, replacing the earlier, simpler picture of electrons being free to orbit at any distance from the nucleus, at any energy.

**Why Shells Alone Weren't the Whole Story: Fine Structure**

As instruments improved — spectrometers capable of much finer resolution — physicists noticed something new: what had first looked like a **single** spectral line often turned out, under closer inspection, to actually be **several extremely closely-spaced lines** bunched tightly together, only visible with sharper equipment. This detail, sometimes called **fine structure**, meant a single shell (one fixed energy level) wasn't the complete picture. There had to be smaller energy differences *within* a shell — splitting what first looked like one energy level into several slightly different sub-levels.

**Where $s$, $p$, $d$, $f$ Actually Came From**

This is the detail most students never get told: the sub-level letters weren't invented from abstract theory — they were originally just labels describing the visual appearance of different types of spectral lines, named well before anyone had a full theoretical explanation for why they existed:

- $s$ = "sharp" — a clean, sharp type of spectral line
- $p$ = "principal" — a strong, prominent line
- $d$ = "diffuse" — a blurrier, more spread-out line
- $f$ = "fundamental" — from a foundational series of lines observed in certain spectra

So these letters started life as descriptive names for patterns of light, and were only later connected to the theoretical concept of separate energy sub-levels sitting inside each shell.

**Confirming It With Math: Quantum Mechanics**

The spectral-line evidence told scientists *that* sub-levels existed — but it took the development of **quantum mechanics**, in the 1920s, to explain *why*, mathematically. Physicists including Erwin Schrödinger and Werner Heisenberg developed equations describing how an electron behaves around a nucleus. Solving the central equation of this theory — the **Schrödinger equation** — for an atom produces solutions that naturally split into groupings matching $s$, $p$, $d$, $f$, each with a distinct shape and energy level, lining up almost perfectly with what the fine-structure spectral lines had already hinted at for decades.

This is also where the true electron-filling order ($1s, 2s, 2p, 3s, 3p, 4s, 3d, 4p, \dots$) ultimately comes from — it isn't an arbitrary sequence to be memorized blindly. It falls directly out of solving these equations for each element, and simply happens to match the order that best explains real, measured spectral data.

**The Two Threads, Brought Together**

The full picture came from two separate threads meeting in the middle:

1. **Experimental observation** — heated atoms producing sharp spectral lines (not smooth rainbows) first suggested electrons occupy fixed energy levels, and closer inspection of "fine structure" within those lines suggested each level was further split into sub-levels.
2. **Theoretical mathematics** — quantum mechanics, developed decades later, explained precisely why those sub-levels exist, what shapes and energies they have, and in what order electrons fill them.

**Key Takeaway**

The $s$, $p$, $d$, $f$ sub-levels weren't invented as abstract theory — they were discovered as an experimental puzzle first. Heated atoms release light only at specific sharp wavelengths rather than as a smooth rainbow, revealing that electron energy comes in fixed, quantized levels (shells). Closer inspection of those spectral lines showed some were actually clusters of several closely-spaced lines — evidence of finer sub-levels within each shell — and those sub-levels were named $s$, $p$, $d$, $f$ based simply on how the corresponding spectral lines looked (sharp, principal, diffuse, fundamental). It was only decades later, with the development of quantum mechanics and the Schrödinger equation, that the mathematics caught up to explain exactly why these sub-levels exist and confirm the energy-ordering pattern that spectral evidence had already been pointing toward.

## Chapter 10 — How Chemists Found Exact Atomic Mass and Atomic Number

*Two different discoveries, decades apart — not one story.*

Atomic mass and atomic number are usually taught side by side as if they were discovered together, in the same way. They weren't. They come from two completely different stories, worked out with different tools, decades apart. This chapter walks through both.

**Atomic Mass: The Older, Cruder Method**

Early chemists worked out **relative atomic masses** by measuring how much of one element reacted with how much of another, then comparing the resulting ratios — all without ever weighing a single atom directly, since no instrument capable of that existed yet. This method gave reasonably good relative numbers (like "oxygen is about 16 times heavier than hydrogen"), but it wasn't fully precise, and crucially, it told chemists nothing about *why* atoms had the masses they did.

**Ordering the Periodic Table by Mass — and the Contradictions That Followed**

For a long stretch of chemistry's history, elements on the periodic table were ordered strictly by **atomic mass**, from lightest to heaviest — because atomic number, as a real physical property, wasn't known to exist yet.

This approach ran into trouble. A handful of elements, when placed strictly by increasing mass, ended up sitting in the wrong chemical family — their reactive behavior didn't match where their weight placed them on the table. Something was clearly inconsistent, but for a long time nobody knew what.

**The Unexpected Fix: X-Rays and Henry Moseley**

The resolution came from a completely different direction than anyone expected. In 1913, physicist **Henry Moseley** fired high-energy electrons at various elements and measured the X-rays each element gave off in response.

He found something remarkable: the frequency of X-rays emitted by an element shifted in a precise, predictable mathematical pattern as he moved from element to element. This pattern corresponded to a specific whole number for each element — a number that increased by exactly 1 every time he moved to the next element along the periodic table.

Moseley realized this whole number represented something more fundamental than atomic mass: it was directly counting **the number of protons in the atom's nucleus** (the term "proton" itself came into common use slightly later — Moseley initially just referred to this integer as the "atomic number," a positive whole number unique to each element).

This discovery resolved the earlier contradictions. The handful of elements that seemed misplaced by mass turned out to have the *correct* atomic number all along — Moseley's X-ray method revealed the true underlying ordering, based on actual nuclear charge (proton count), rather than weight.

**Why Atomic Number, Not Atomic Mass, Defines an Element**

This was a genuinely significant correction, because it meant atomic number — not atomic mass — is what truly defines which element something is.

Two atoms of the exact same element can actually carry slightly different masses. These are called **isotopes**: atoms with the same number of protons (same atomic number, same element) but a different number of neutrons (different mass). Mass, it turns out, can vary somewhat for atoms of the same element — but atomic number cannot, since it's fixed entirely by proton count. That's precisely why the modern periodic table is organized by atomic number, not atomic mass — a direct legacy of Moseley's X-ray experiments correcting the earlier mass-based ordering.

**How Exact Atomic Mass Is Measured Today**

Modern atomic mass measurement relies on an instrument called a **mass spectrometer**. In simplified terms, the process works like this:

1. A sample of atoms is turned into charged particles (ions) by knocking off an electron from each atom.
2. These ions are fired through a magnetic field.
3. The magnetic field bends each ion's path by an amount that depends on its mass — heavier ions bend less, lighter ions bend more, under the same magnetic force.
4. The precise amount of bending is used to calculate the ion's exact mass.

This method gives real, absolute atomic masses (the incredibly tiny gram-scale values discussed elsewhere — around $1.67 \times 10^{-24}$ grams for a single hydrogen atom), not just relative ratios calculated from reaction data.

**Why Atomic Mass on the Periodic Table Is Usually a Decimal, Not a Clean Whole Number**

Mass spectrometry also revealed something else important: most elements found in nature aren't made of atoms that are all identical in mass — they're actually a mix of several isotopes, each occurring in a certain natural proportion, each with a slightly different mass. The atomic mass value printed on the periodic table today is therefore usually a **weighted average** across an element's naturally occurring isotopes, rather than the mass of any single, individual atom. This is why atomic mass numbers are so often decimals (like chlorine's 35.45) rather than clean whole numbers — they represent a blend of multiple real isotope masses, weighted by how common each one is in nature.

**Key Takeaway**

Atomic mass and atomic number were discovered through entirely different methods. Atomic mass came first, worked out through reaction-ratio comparisons that gave only relative values, later replaced by precise absolute measurements from mass spectrometers. Atomic number came much later, in 1913, when Henry Moseley's X-ray experiments revealed a whole-number pattern tied directly to the number of protons in an atom's nucleus — a discovery that corrected earlier contradictions in the mass-ordered periodic table and established atomic number, not atomic mass, as the true defining property of an element. Because most elements are natural mixtures of multiple isotopes with slightly different masses, the atomic mass shown on the periodic table today is a weighted average across those isotopes, which is why it usually appears as a decimal rather than a clean whole number.
