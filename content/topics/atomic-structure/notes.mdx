...
- Left side: 2 units of $\text{H}_2$ = 4 hydrogen atoms. 1 unit of $\text{O}_2$ = 2 oxygen atoms.
- Right side: those same 4 hydrogen atoms and 2 oxygen atoms need to be packaged into water molecules. Each water molecule ($\text{H}_2\text{O}$) uses 2 hydrogen atoms and 1 oxygen atom. So 4 H atoms + 2 O atoms build exactly **2 complete water molecules** — no more, no less.
...
Since "2 units" of water corresponds to 2 liters (using the equal-particles rule), the final gas volume is 2 liters — smaller than the 3 liters you started with. The volume shrank because 3 separate starting molecules ($2\text{H}_2 + 1\text{O}_2$) recombined into only 2 separate product molecules ($2\text{H}_2\text{O}$). Fewer total molecules on the product side means fewer liters, even though every single atom is still accounted for.
...
This matches the real, balanced chemical equation exactly. Checking the atoms confirms it: 1 $\text{N}_2$ molecule has 2 nitrogen atoms; 2 $\text{NH}_3$ molecules also need 2 nitrogen atoms total. 3 $\text{H}_2$ molecules have 6 hydrogen atoms; 2 $\text{NH}_3$ molecules need 6 hydrogen atoms total (3 each). Both sides balance perfectly.
